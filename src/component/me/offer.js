...
class Offer extends Component{
	...
	componentDidMount=()=>{
		api.FetchGet(`/hyb-stu/stu_written/offer/${this.props.location.state.id}`,{
			...
			this.setState({ 
				...
				if(new Date(res.data.expire_date.replace(/-/g,"/")) < new Date(res.data.server_time.replace(/-/g,"/"))){
					api.tip_msg('笔试邀请函已过期','fixedmode');
					setTimeout(()=>{
		...
		return(
			<div className="offer">
				...
				<div className="bd">
					...
					<br/>
					<p className="kg"><span>{this.state.full_name}</span> 邀请你参加 <span>{this.state.position_name}</span> 在线笔试</p>
					<p className="kg">请在下方选择最适合您的笔试时间，{api.settime(this.state.expire_date)} 前未选择将视为放弃。</p>
					{
				...
				<div className="ft">
					{
						this.state.choose_time ? <b>已选笔试时间：{api.settime(this.state.choose_time)}</b> : ''
					}
					<Link to={{ "pathname" : "/selecttimew" ,state : { "expiredate" : this.state.expire_date ,"positionid" : this.props.location.state.id ,"entid" : this.state.ent_id ,"planid" : this.state.plan_id }}}>
						<span className="totime">{this.state.choose_time ? '更改笔试时间' : '请选择笔试时间'}</span>
					</Link>
					<em>
