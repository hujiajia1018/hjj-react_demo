...
class CreationSocial extends Component{
	constructor(props){
		super(props);
		this.state = {
			hot_point : {},
	...
	scroll=(event)=>{
		...
		if(!el || !this.remain || this.state.hfstatus=='pending'){
			return;
		}
		...
		if(this.state.status!='pending'){
			if(scrolltop + window.innerHeight + 10 >= Math.ceil(document.body.scrollHeight)){
				++this.page;
				this.setState({ "hfstatus" : 'pending'},()=>{
					setTimeout(()=>{
						this.getdata();
					},500);
				});
	...
	showpage=()=>{
		return  <div>
					<div className="box" data-time={this.state.hot_point.create_time} data-id={this.state.hot_point.hot_point_id}>
						...
						<div className="boxbd">
							<p onClick={this.creatcomment} data-showtype="0">{this.state.hot_point.article_content.content}</p>
							<ul className="imglist">
								<li className="long">
									{
										this.state.hot_point.pictures ? <Link to={{ "pathname" : "/creationdetail" ,state : { "hotpointid" : this.state.hot_point.hot_point_id ,"name" : this.state.hot_point.name ,"icon" : this.state.hot_point.icon ,"userid" : this.state.hot_point.user_id } }}><img src={this.state.hot_point.pictures[0]} /></Link> : ''
									}
									<Link to={{ "pathname" : "/creationdetail" ,state : { "hotpointid" : this.state.hot_point.hot_point_id ,"name" : this.state.hot_point.name ,"icon" : this.state.hot_point.icon ,"userid" : this.state.hot_point.user_id } }}>
										<p>
											<em>{this.state.hot_point.title.length > 30 ? api.substring(this.state.hot_point.title ,30 ,'...') : this.state.hot_point.title}</em>
											{this.state.hot_point.title.length > 30 ? <span>【长文】</span> : ''}
										</p>
									</Link>
								</li>
							</ul>
					...
					<div className="dynamicdetail">
						{
							...
							: this.state.comment_list.map((item ,index)=>{
								return <div className="box" key={index} >
											...
											<div className="right">
												<h2>{ (item.comment_mongo_info.anonymous_type==1 && item.comment_mongo_info.anonymous_name) ? item.comment_mongo_info.anonymous_name : item.comment_mongo_info.name }</h2>
						...
						{ this.state.hfstatus=='pending' ? <Tipshowend text="加载中请稍等"/> : ''}
					</div>
				</div>
	}
	goback=()=>{
	...
	render(){
		return(
			<div className="merelease">
				...
				{
					this.state.status=='pending' ? <Tip text="加载中请稍等" type="loading" /> : this.state.status=="success" ? this.showpage() : <Tip text="出错误了" type="tiperro" />
				}
...
CreationSocial = connect(mapStateToProps)(CreationSocial)
